...
function makeCrossPlatform(command) {
  ...
  return command.split("&&").map((subcommand) => {
...
var TaskRuntime = class _TaskRuntime {
  static {
     ...
     */
    this.MANIFEST_FILE = path.posix.join(
      PROJEN_DIR,
      "tasks.json"
    );
  }
...
var RunTask = class {
  constructor(runtime, task, parents = [], args = [], envParam = {}) {
    ...
    if (!task.steps || task.steps.length === 0) {
      this.logDebug((0, import_chalk.gray)("No actions have been specified for this task."));
      return;
    }
    ...
    if (envlogs.length) {
      this.logDebug((0, import_chalk.gray)(`${(0, import_chalk.underline)("env")}: ${envlogs.join(" ")}`));
    }
  ...
  evalCondition(taskOrStep) {
    ...
    this.log((0, import_chalk.gray)(`${(0, import_chalk.underline)("condition")}: ${taskOrStep.condition}`));
    const result = this.shell({
  ...
  fmtLog(...args) {
    return (0, import_util.format)(`${(0, import_chalk.underline)(this.fullname)} |`, ...args);
  }
  shell(options) {
